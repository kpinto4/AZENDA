...
export class PublicController {
  ...
  getPublicAvailability(@Param('slug') slug: string, @Query('date') date: string) {
    return this.booking.getPublicAvailability(slug, date);
  }

  @Post(':slug/appointments')
  @HttpCode(HttpStatus.CREATED)
  createBooking(@Param('slug') slug: string, @Body() dto: CreatePublicAppointmentDto) {
    return this.booking.createBooking(slug, dto);
  }

  @Post(':slug/reprogramar-cita')
  @HttpCode(HttpStatus.OK)
  reprogramarCita(@Param('slug') slug: string, @Body() dto: ReschedulePublicAppointmentDto) {
  ...
  @HttpCode(HttpStatus.OK)
  buscarCitasActivas(@Param('slug') slug: string, @Body() dto: LookupPublicAppointmentsDto) {
