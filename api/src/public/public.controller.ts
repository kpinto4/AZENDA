...
/** Inicio de la cita en ms (hora local del servidor) a partir de `when_at` tipo `YYYY-MM-DD HH:mm`. */
...
/** Antelación mínima del cliente para reprogramar (1,5 h antes del inicio de la cita actual). */
...
export class PublicController {
  ...
  async reprogramarCita(@Param('slug') slug: string, @Body() dto: ReschedulePublicAppointmentDto) {
    const tenant = await this.sqlDb.findTenantBySlug(slug);
    if (!tenant) {
      throw new NotFoundException('Negocio no encontrado');
    }
  ...
  @HttpCode(HttpStatus.OK)
  async buscarCitasActivas(@Param('slug') slug: string, @Body() dto: LookupPublicAppointmentsDto) {
